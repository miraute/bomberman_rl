...
def setup(self):
    ...
    self.file_weights = 'weights_simple.pkl'
    try:
        #weights = load_weights(self.file_weights)       # load trained weights
        weights = torch.load('weights_simple.pt')
        ...
        pass
    try:
        optimizer_state = torch.load('opt_simple.pt')
    ...
    self.pnet = DQN(s.cols, s.rows,6).to(device)          # initialize policy network...
    if weights:
        self.pnet.load_state_dict(weights)              #  ... with trained weights (if they already exist)
    self.tnet = DQN(s.cols, s.rows,6).to(device)          # initialize target network
...
def get_state(self):
    ...
    state = np.ones((1,17,17))*5                    # init array
    ...
    if len(self.game_state['others']) > 0:
        ...
        state[0,others_x,others_y] = -4             # set positions of opponents
...
# old stack: torch.tensor with shape (1,6,h,w) or torch.from_numpy(np.array([np.nan]))
# state:     torch.tensor with shape (1,1,h,w)
# returns    torch.tensor with shape (1,6,h,w) where ret[0,0] = state and the rest moved 1 index to the back
#                or in the case that old_stack == nan: a stacked version of 6 times state
def stack(old_stack, state):
    if torch.any(torch.isnan(old_stack)):
        stacked = torch.zeros(tuple((6,state.shape[-2],state.shape[-1])),dtype=state.dtype,device=state.device)
        for i in range(6):
            stacked[i,:,:] = state
        return stacked.unsqueeze(0)
    for i in range(5,0,-1):
        old_stack[0,i,:,:]=old_stack[0,i-1,:,:]
    old_stack[0,0,:,:] = state
    return old_stack
...
def act(self):
    ...
    #valid_actions = get_valid_action(self, state_np)      # determine valid actions...
    valid_actions = list(s.actions)                         # ... or don't determine valid actions
        ...
        if event in [0,1,2,3]:          # MOVED left, right, up or down
            ...
            for p in [(min(x+2,16),y), (max(x-2,0),y),(x,min(y+2,16)),(x,max(y-2,0)),(x+1,y+1),(x+1,y-1),(x-1,y+1),(x-1,y-1)]:
                if p in [(x,y) for (x,y,n,b,s) in self.game_state['others']]:
                    ...
                    break
            if invalid_action:
                self.logger.debug('-------- INVALID ACTION ----------- this should not happen!')
                r -= 1
...
def end_of_episode(self):
    ...
    torch.save(weights, 'weights_simple.pt')
    torch.save(self.optimizer.state_dict(),'opt_simple.pt')
