...
if True:
    max_rows = 15000 #None
    reward_sum, reward_mean = genfromtxt('out.csv', delimiter=',',unpack=True,max_rows=max_rows)
    ...
    weights = genfromtxt('weights.csv', delimiter=',',max_rows=max_rows)
    plt.plot(np.arange(weights.shape[0]), weights, alpha=.7)
    plt.xlabel('steps')
    plt.ylabel('weights')
    plt.legend(np.arange(weights.shape[1]),loc='upper left')
